...
export function InvoiceIntelligence({ notify, mode, initialInvoices = [], onInvoicesChange }: { notify: (message: string) => void; mode: "demo" | "real"; initialInvoices?: Invoice[]; onInvoicesChange?: (invoices: Invoice[]) => void }) {
  ...
  const updateDocument = (id: string, patch: Partial<UploadedDocument>) => setDocuments((current) => {
    ...
    const flattened = next.flatMap((document) => document.invoices);
    invoicesRef.current = flattened; setInvoicesState(flattened); onInvoicesChange?.(flattened);
    return next;
  });
  ...
  const start = async () => {
    if (!fileEntries.length || running) return; setRunning(true); const controller = new AbortController(); abortRef.current = controller;
    await processInvoiceQueue(fileEntries.map((entry) => entry.file), fileEntries.map((entry) => entry.documentId), concurrency, controller.signal, updateDocument); setRunning(false); notify(controller.signal.aborted ? "Procesamiento cancelado" : "Lote analizado");
  };
  ...
  return <div className="invoice-intelligence">
    ...
    <div className="queue-toolbar"><div><b>{documents.length || invoices.length} archivo(s) · {invoices.length} factura(s) detectada(s)</b><span>{completed} completadas · {reviews} requieren revisión · {errors} con error · {documents.filter((i) => i.status === "Procesando").length} documentos procesando</span></div><label>Procesar simultáneamente <select value={concurrency} onChange={(event) => setConcurrency(Number(event.target.value))}>{[1,2,3,4,5].map((value) => <option key={value}>{value}</option>)}</select></label>{running ? <button className="secondary danger" onClick={() => abortRef.current?.abort()}><Ban size={15}/> Cancelar</button> : <button className="primary" disabled={!fileEntries.length} onClick={start}><Play size={15}/> Iniciar análisis</button>}</div>
    {documents.length > 0 && <div className="invoice-table-wrap"><table><thead><tr><th>Documento</th><th>Páginas</th><th>Facturas detectadas</th><th>Estado</th><th>Progreso</th></tr></thead><tbody>{documents.map((document) => <tr key={document.id}><td className="strong">{document.fileName}<small className="sub">{size(document.fileSize)}</small></td><td>{document.pageCount || "—"}</td><td>{document.detectedInvoices}</td><td><span className={"badge " + tone(document.status)}>{document.status}</span></td><td><div className="invoice-progress"><i style={{ width: document.progress + "%" }}/><small>{document.progressLabel}</small></div></td></tr>)}</tbody></table></div>}
