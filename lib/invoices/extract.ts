...
import { extractKerryLayout } from "./kerry-layout";
...
export function extractInvoice(text: string, metadata: Pick<Invoice, "id" | "fileName" | "fileSize" | "sourceType" | "pageCount">, tokens: PdfTextToken[] = [], expectedDocumentType?: "invoice" | "delivery_note"): Invoice {
  const kerry = expectedDocumentType === "delivery_note" ? null : extractKerryLayout(text, tokens);
  ...
  const positionalItems = kerry?.items.length ? kerry.items : extractPositionalItems(tokens);
  ...
  if (kerry && kerry.confidence >= 5) {
    result.date = field(kerry.originalDateTime, kerry.date, confidence(kerry.date)); result.emissionTimestamp = kerry.originalDateTime ?? undefined;
    result.supplier = field(kerry.supplier, kerry.supplier, confidence(kerry.supplier)); result.supplierId = field(kerry.supplierTaxId, kerry.supplierTaxId, confidence(kerry.supplierTaxId));
    result.internalDocument = field(kerry.internalDocument, kerry.internalDocument, confidence(kerry.internalDocument)); result.orderNumber = field(kerry.orderNumber, kerry.orderNumber, confidence(kerry.orderNumber)); result.deliveryNumber = field(kerry.deliveryNumber, kerry.deliveryNumber, confidence(kerry.deliveryNumber)); result.purchaseOrder = field(kerry.purchaseOrder, kerry.purchaseOrder, confidence(kerry.purchaseOrder));
    result.currency = field(kerry.currency, kerry.currency, confidence(kerry.currency)); result.exchangeRate = field(kerry.exchangeRate == null ? null : String(kerry.exchangeRate), kerry.exchangeRate, confidence(kerry.exchangeRate)); result.incoterm = field(kerry.incoterm, kerry.incoterm, confidence(kerry.incoterm)); result.totalGrossWeight = field(kerry.totalGrossWeight == null ? null : String(kerry.totalGrossWeight), kerry.totalGrossWeight, confidence(kerry.totalGrossWeight)); result.totalNetWeight = field(kerry.totalNetWeight == null ? null : String(kerry.totalNetWeight), kerry.totalNetWeight, confidence(kerry.totalNetWeight)); result.items = kerry.items;
  }
  return result;
}
